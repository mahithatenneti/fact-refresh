...
def _fetch_ok(url: str) -> bool:
    ...
    """
    try:
        r = requests.get(url, timeout=REQUEST_TIMEOUT, headers={**HEADERS, "Accept": "*/*"}, stream=True)
...
def _extract_article_text(url: str) -> str:
    ...
    try:
        ...
        candidates = []
        for sel in [
            "article", ".story__content", ".content", ".article-content", ".row .col",
            "#content", ".Post-content", ".td-post-content", ".entry-content", ".story-content",
            "main"
        ]:
        ...
        try:
            ...
            for e in parsed.entries[: MAX_PER_SOURCE * 3]:  # overfetch; we filter below
                ...
                items.append({
                    ...
                    "url": link.strip(),
                    "source": source_name,
                    "date": getattr(e, "published", "") or getattr(e, "updated", "") or _utcnow_iso(),
                    "lang": default_lang,
                })
...
def scrape_eenadu() -> List[Dict]:
    # If this ever breaks, use Google News fallback (below).
    feeds = [
